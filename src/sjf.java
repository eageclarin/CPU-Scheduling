...
/** schedule processes **/
class shortestJobFirst extends TimerTask {
    private final process process[]; static int isNotP;
    int time = 0; int currentBT, currentID, prevID = Integer.MAX_VALUE;

    shortestJobFirst (process process[], boolean isNotPreemptive) {
    ...
    private void completeTask(int time) {
        try {
            //traverse through processes with burstTime > 0, compare shortest burstTime
            for (int i=0; i<process.length; i++) {
                if (shortestJobFirst.isNotP == 0) { //preemptive
                    if (process[i].aT <= time && process[i].bT > 0 && process[i].bT < currentBT) {
                        currentBT = process[i].bT; currentID = i;
                    }
                } else { //non-preemptive
                    if (process[i].bT > 0 && process[i].bT < currentBT) {
                        currentBT = process[i].bT; currentID = i;
                    }
                }
            }
...
public class sjf {
    ...
    public void startSchedule(process process[], int completionTime, boolean isNotPreemptive) {
        ...
        timer.schedule(new shortestJobFirst(process, isNotPreemptive), 0, 1000); //run task every 1 second
    ...
    public void schedule(ArrayList<Integer> p, ArrayList<Integer> burstTime, ArrayList<Integer> arrivalTime) {
        ...
        if (!isNotPreemptive) { //if preemptive
            ...
            Arrays.sort(process, new sortByBurst());
